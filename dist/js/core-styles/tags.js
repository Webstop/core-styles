  ...
  function renderTagSearchResults(data, tagging){
    ...
    $.each( data, function( position, record ) {
      $.each( record, function( key, value ) {
        if(key == 'name'){
          items.push(`<button type="button" class="list-group-item list-group-item-action" data-tag-add data-tag-search-target="${tagging.target}" data-tag-search-display="${tagging.display}" data-tag-search-results="${tagging.results}">${value}</button>`);
        }
      });
    });
